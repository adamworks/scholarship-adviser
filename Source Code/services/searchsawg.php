...
if($_GET['act'] == 'grab')
{
	...
	// $grab = new Grab_link($web, $kat, $page, $jmlh_page);
	// $link = $grab->routing();
	...
	if($update == '0'){
		...
		$beasiswa = filtersearch($kategori_id);

		//header('Content-type: application/json');
		echo '{"items":'.json_encode($beasiswa).'}';
	}
	else if($update == '1'){
		$grab = new yqlsearch($kat,$jur,$neg);
		...
		for ($row = 0; $row < count($link); $row++)
			{
						...
						save_konten($link[$row]['domain'],$kategori_id, $link[$row]['alamat'], $link[$row]['judul'], $link[$row]['deadline'], $link[$row]['univ'], $link[$row]['negara'],$link[$row]['konten'],$link[$row]['jurusan']);
						//save_konten2($link[$row]['domain'],$kategori_id, $link[$row]['alamat'], $link[$row]['judul'], $link[$row]['deadline'], $link[$row]['univ'], $link[$row]['negara'],$link[$row]['konten'],$link[$row]['jurusan']);
						//save _img($result['image'],$imgName);
					echo "sukses";
			}		
		
		//$searchsaw = $grab->routing();
...
class yqlsearch
{
	...
	public function __construct($kat,$jur,$neg)
	{
	...
	public function routing($i)
	{
		...
		elseif($i == 2)
		{
			$result = $this->scholarship2015();
		}
		elseif($i == 3)
		{
			$result = $this->beasiswa2014();
		}
		else
		{
	...
	public function scholarshipdb()
	{
		...
		$page = 1;
		$jmlh_page = 2;
		$result = array(array('domain'=>'','alamat'=>'','judul'=>'','deadline'=>'','univ'=>'','negara'=>'','konten'=>'','jurusan'=>''));
		...
		while($page<=$jmlh_page)
		{
			...
			if($kat == 's1')
			{
			    ...
			    for ($i=0; $i< count($sch); $i++){  
						...
						//$jurnew = mysql_escape_string($this->jur);
			...
			elseif($kat == 's3')
			{
			    ...
			    for ($i=0; $i< count($sch); $i++){  
						...
						$deadline = $data->query->results->results[0]->span[1]->content;
						$deadline2 = mysql_escape_string($deadline);
						$result[$i]['deadline'] = $deadline2;
