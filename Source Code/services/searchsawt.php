...
if($_GET['act'] == 'grab')
{
	...
	//}
		...
		for ($row = 0; $row < count($link); $row++)
			{
					$beasiswa = save_konten($link[$row]['domain'],$kategori_id, $link[$row]['alamat'], $link[$row]['judul'], $link[$row]['deadline'], $link[$row]['univ'], $link[$row]['negara'],$link[$row]['konten'],$link[$row]['jurusan']);
...
class yqlsearch
{
	...
	function alamat_website_kategori($id_website){
		...
		$sql = mysql_query("SELECT id_media FROM media WHERE id_media = '$id_website'");
