...
public class CalendarEventPlugin extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray data, CallbackContext callbackContext) throws JSONException {
        if (action.equals("addCalendarEvent")) {
            if(data.length() > 0)
            {
                //if you are passing some data eg. start,end date, all day and parameters for the nl.xservices.plugins.Calendar
    ...
    private boolean addCalendarEvent(){
        ...
        try{
            ...
            beginTime.set(2015, 5, 25, 7, 30);
            Calendar endTime = Calendar.getInstance();
            endTime.set(2015, 5, 26, 8, 30); //set your own time here and above as well or you can get the current time.
